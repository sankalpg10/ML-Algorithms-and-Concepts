"""Building an autoencoder with a single fully connected neural network with a sparsity constraint on the encoded representation.
a sparser model can be desirable in an autoencoder for dimensionality reduction because it can lead to more efficient and 
effective encoding of the input data, which can help to prevent overfitting and improve generalization performance.
This sparser model is able to achieve a similar level of compression as the previous model in simpleautoencoder.py with only half as many active neurons.
...
encoded = layers.Dense(
    encoded_dim, activation="relu", activity_regularizer=regularizers.l1(10e-5)
)(
    input_image
)  # linear activation for simple encoder
...
autoencoder.fit(
    x=x_train, y=x_train, epochs=100, batch_size=256, validation_data=(x_test, x_test)
)
